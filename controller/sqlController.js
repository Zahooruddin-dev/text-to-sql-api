...
exports.generateAndRunSQL = async (req, res) => {
	...
	try {
			...
			messages: [
				{
					role: 'system',
					content:
						'You are a specialized SQL assistant. Database: Postgres. Table: users(id, name, email). Return ONLY the SQL string. No conversational text.',
				},
		...
		res
			.status(500)
			.json({ error: 'Failed to generate or run SQL', details: err.message });
	}
};
