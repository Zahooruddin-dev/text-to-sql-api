...
const sqlRoutesV5 = require('./routes/sqlRoutesV5');
...
app.get('/health', (req, res) => res.json({ 
  status: 'ok', 
  apiVersion: 'v5',
  supportedVersions: ['v1', 'v2', 'v3', 'v4', 'v5']
}));

// In-memory metrics endpoint (V2)
app.get('/metrics', (req, res) => res.json(getMetricsSnapshot()));

// V5 API Routes (JWT/OAuth authenticated)
app.use('/api/v5', sqlRoutesV5);
...
app.use('/api/v1', (req, res, next) => {
  ...
  res.setHeader('Link', '</api/v5>; rel="successor-version"');
  next();
}, sqlRoutes);
...
app.use((req, res) => {
  res.status(404).json({
    ...
    supportedVersions: ['/api/v5', '/api/v4', '/api/v3', '/api/v2', '/api/v1']
  });
});
...
if (require.main === module) {
	app.listen(PORT, () => {
  console.log(`Text-to-SQL API V5 running on port ${PORT}`);
  console.log(`API V5 (JWT/OAuth): http://localhost:${PORT}/api/v5`);
